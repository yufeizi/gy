...
class SimpleBitBrowserGUI:
    ...
    def __init__(self):
        ...
        self.api = None
        self.bitbrowser_api = None  # 🔥 添加bitbrowser_api属性，与api指向同一实例
        ...
        self.global_memory_threshold = 600  # 默认内存阈值600MB
            ...
            if os.path.exists(self.config_file):
                with open(self.config_file, 'r', encoding='utf-8') as f:
                    ...
                    if 'parse_settings' in config:
                        ...
                        self.global_memory_threshold = settings.get('memory_threshold', 600)
    ...
    def _load_filter_keywords_from_file(self):
        ...
        try:
            ...
            self.logger.error(f"❌ 加载过滤关键词失败: {e}")
    ...
    def show_account_manager(self):
        ...
        ttk.Button(btn_frame, text="❌ 关闭", command=account_window.destroy).pack(side=tk.RIGHT)
    ...
    def show_backup_manager(self):
        ...
        ttk.Button(btn_frame, text="❌ 关闭", command=backup_window.destroy).pack(side=tk.RIGHT)
    ...
    def create_control_section(self, parent):
        ...
        # 开始/停止解析按钮（动态切换）
        self.start_stop_button = tk.Button(parse_frame, text="▶开始解析", command=self.toggle_start_stop, 
                                          bg="#2E8B57", fg="white", font=("宋体", 10), 
                                          relief="flat", bd=0, anchor="center", width=12, height=2)
        ...
        # 暂停/继续程序按钮（动态切换）
        self.pause_continue_button = tk.Button(parse_frame, text="⏸暂停程序", command=self.toggle_pause_continue, 
    ...
    def create_browser_list(self, parent):
        ...
        self.browser_tree.heading('runtime', text='运行时长')  # 🔥 修改：解析状态改为运行时长
        ...
        self.browser_tree.column('runtime', width=100)  # 🔥 修改：运行时长列
        ...
        def run():
            try:
                ...
                if not api_token:
                    ...
                    self.log_message("❌ 错误：请输入API Token")
                ...
                if len(cleaned_token) < 16:
                    ...
                    self.log_message("❌ 错误：API Token格式无效")
                ...
                self.bitbrowser_api = self.api  # 🔥 让bitbrowser_api指向同一个实例
                ...
                if self.api.test_connection():
                    ...
                    if browsers is not None:
                        ...
                        self.connection_status.config(text="❌ Token无效", foreground="red")
                        self.log_message("❌ API Token无效，请检查Token是否正确")
                        messagebox.showerror("Token错误", "API Token无效！\n\n请检查：\n1. Token是否正确复制\n2. 比特浏览器是否开启API功能\n3. Token是否已过期")
                else:
                    self.connection_status.config(text="❌ 服务异常", foreground="red")
                    self.log_message("❌ 比特浏览器本地服务连接失败")
                    messagebox.showerror("连接失败", "无法连接到比特浏览器本地服务！\n\n请检查：\n1. 比特浏览器软件是否正在运行\n2. 本地服务端口是否正常（从配置文件获取）\n3. 防火墙是否阻止连接")

            except Exception as e:
                self.connection_status.config(text="❌ 连接错误", foreground="red")
        ...
        def run():
            try:
                ...
                if browser_id:
                    ...
                    self.log_message(f"❌ 实例创建失败: {name}")
    ...
    def update_browser_display(self):
        ...
        for index, (browser_id, browser) in enumerate(sorted_browsers, 1):
            ...
            if has_error:
                takeover_status = "❌ 错误"  # 红色X表示错误
            elif is_paused:
                takeover_status = "⏸️ 已暂停"  # 显示暂停状态
            elif is_stopped:
                ...
                for browser_id, browser in self.browsers.items():
                    if not browser.get('is_running', False):
                        ...
                        if result:
                            ...
                            self.log_message(f"❌ 开启失败: {browser.get('name')}")
                ...
                for browser_id, browser in self.browsers.items():
                    if browser.get('is_running', False):
                        ...
                        if result:
                            ...
                            self.log_message(f"❌ 关闭失败: {browser.get('name')}")
        ...
        try:
            ...
            if item_id:
                ...
                if browser:
                    ...
                    self.log_message(f"❌ 未找到浏览器数据: {browser_name}")
                    self.log_message(f"可用的浏览器: {[b.get('name') for b in self.browsers.values()]}")
            else:
                self.log_message("❌ 未识别到有效的项目")

        except Exception as e:
            self.log_message(f"❌ 右键菜单错误: {e}")
            import traceback
    ...
    def show_context_menu(self, event, browser):
        ...
        try:
            ...
            if is_browser_running:
                ...
                if is_paused:
                    # 暂停状态：显示继续程序
                    context_menu.add_command(label="▶️ 继续程序", command=lambda: self.continue_program(browser))
                elif is_stopped:
                    # 停止状态：显示开始程序
        ...
        def run():
            try:
                ...
                if self.account_manager.backup_browser_config(self.api, browser['id'], browser.get('name')):
                    ...
                    self.log_message(f"❌ 浏览器配置备份失败: {browser.get('name')}")
...
"""
                ...
                for attempt in range(max_retries):
                    ...
                    if result:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            self.log_message(f"❌ 浏览器打开失败: {browser.get('name')} (已重试{max_retries}次)")
        ...
        def run():
            try:
                ...
                if self.api.close_browser(browser['id']):
                    ...
                    self.log_message(f"❌ 浏览器关闭失败: {browser.get('name')}")
        ...
        def run():
            try:
                ...
                if script_processes:
                    ...
                    self.log_message(f"ℹ️ 未找到运行中的脚本进程: {browser.get('name')}")
                ...
                try:
                    if process.info['name'] and 'python' in process.info['name'].lower():
                        ...
                        if cmdline:
                            ...
                            # 🔥 方法4: 检查进程是否在generated_scripts目录下运行
                            try:
                                if hasattr(process, 'cwd'):
                                    cwd = process.cwd()
            ...
            try:
                ...
                if script_processes:
                    ...
                    if paused_count > 0:
                        ...
                        self.log_message(f"ℹ️ 没有找到需要暂停的脚本进程")
                else:
                    self.log_message(f"ℹ️ 未找到运行中的脚本进程，继续启动手动解析")
                ...
                try:
                    ...
                    if success:
                        ...
                        self.log_message(f"❌ 浏览器 {browser_id_short} 的手动解析启动失败")
                except Exception as e:
    ...
    def _start_unified_manual_extraction(self, browser_id: str) -> bool:
        ...
        try:
            ...
            self.log_message(f"❌ 统一手动解析启动失败: {e}")
            return False
    ...
    def _start_manual_extraction_for_browser(self, browser_id: str) -> bool:
        ...
        try:
            ...
            if not os.path.exists(sd_path):
                self.log_message(f"❌ 找不到sd.py文件: {sd_path}")
                return False
            ...
            if success:
                ...
                self.log_message(f"❌ 浏览器 {browser_id[-6:]} 的手动解析启动失败")
                return False
                            
        except Exception as e:
            self.log_message(f"❌ 启动浏览器 {browser_id[-6:]} 手动解析失败: {e}")
            return False
    ...
    def _stop_manual_extraction_for_browser(self, browser_id: str) -> bool:
        ...
        try:
            ...
            self.log_message(f"❌ 停止浏览器 {browser_id[-6:]} 手动解析失败: {e}")
            return False
    ...
    def start_auto_extraction(self, browser):
        ...
        def run():
            try:
                ...
                # 恢复暂停的自动化任务
                self.log_message(f"▶️ 恢复浏览器自动化任务: {browser.get('name')}")
                script_processes = self._find_browser_script_processes(browser_id)
            ...
            if os.path.exists(script_path):
                ...
                try:
                    ...
                    if process.poll() is None:  # 进程还在运行
                        ...
                        self.log_message(f"❌ 自动化脚本启动失败，返回码: {return_code}")
                        # 尝试读取错误信息
                        try:
                ...
                self.log_message(f"❌ 自动化脚本不存在: {script_path}")
                ...
                if hasattr(self, 'manual_extraction_processes') and browser_id in self.manual_extraction_processes:
                    ...
                    try:
                        if process.poll() is None:  # 进程还在运行
                            ...
                            self.log_message(f"ℹ️ 手动抓取进程已结束: PID {process.pid}")
                    ...
                    self.log_message(f"ℹ️ 未找到手动抓取进程: {browser.get('name')}")
    ...
    def stop_all_manual_extraction(self):
        ...
        def run():
            try:
                ...
                if not hasattr(self, 'manual_extraction_processes'):
                    self.log_message("ℹ️ 没有运行中的手动解析进程")
                    return
                ...
                # 🔥 简化：没有手动解析进程时不显示日志，避免干扰
                    
            except Exception as e:
                self.log_message(f"❌ 停止所有手动解析失败: {e}")
        ...
        def run():
            try:
                ...
                if self.api.delete_browser(browser['id']):
                    ...
                    self.log_message(f"❌ 浏览器删除失败: {browser.get('name')}")
        ...
        try:
            ...
            try:
                ...
                if os.path.exists(log_file_path):
                    ...
                    self.log_message("ℹ️ browser_control.log文件不存在")
            except Exception as e:
    ...
    def _stop_all_scripts_on_exit(self):
        """程序退出时停止所有运行的脚本"""
        try:
            # 🔥 修复：直接调用stop_parsing_scripts来真正终止进程
    ...
    def _force_terminate_all_scripts(self):
        ...
        try:
                                ...
                                if 'main.py' not in cmdline_str and 'simple_gui.py' not in cmdline_str:
                                    try:
                                        ...
                                        try:
                                            proc.kill()
                                            terminated_count += 1
                                            self.log_message(f"🔥 强制终止进程 PID: {proc.pid}")
                                        except:
                                            # 最后使用taskkill
                                            try:
            ...
            if terminated_count > 0:
                ...
                self.log_message("ℹ️ 没有找到需要终止的脚本进程")
                
        except Exception as e:
            self.log_message(f"❌ 强制终止脚本失败: {e}")
...
6. 示例：100-1000 表示只处理价格在100到1000元之间的商品"""
    ...
    def create_save_buttons(self, parent_window):
        ...
        ttk.Button(right_frame, text="保存所有设置", command=self.save_all_settings).pack(side=tk.RIGHT, padx=(5, 0))
    ...
    def save_all_settings(self):
        ...
        try:
            ...
            # 显示保存成功状态
            self.save_status_label.config(text="✅ 设置保存成功！", foreground="green")
            self.root.after(3000, lambda: self.save_status_label.config(text="✅ 设置已自动保存", foreground="green"))

            messagebox.showinfo("保存成功", "所有设置已保存成功！\n这些设置将用于生成浏览器脚本。")
                ...
                if browser_folder.is_dir() and browser_folder.name.startswith('browser_'):
                    ...
                    for filter_file in filter_files:
                        try:
                            ...
                            print(f"   ❌ 更新失败 {filter_file.name}: {e}")
    ...
    def clear_all_filter_keywords(self):
        ...
        try:
            ...
            self.log_message(f"❌ 清除过滤关键词失败: {e}")
    ...
    def _clear_searched_keywords_file(self):
        ...
        try:
            ...
            self.log_message(f"❌ 清除已搜索关键词文件失败: {e}")
    ...
    def update_searched_keywords_display(self):
        """🔥 统一方法：更新已搜索关键词显示（已废弃，使用refresh_searched_keywords_display）"""
        ...
        def run_generation():
            try:
                ...
                if not running_browsers:
                    self.log_message("❌ 没有运行中的浏览器实例")
                    messagebox.showwarning("警告", "没有运行中的浏览器实例")
                    return

                # 检查是否设置了搜索关键词
                if not self.global_search_keywords:
                    self.log_message("❌ 请先设置搜索关键词")
                    messagebox.showwarning("警告", "请先设置搜索关键词")
                    return

                # 🔥 检查是否有待搜索的关键词（过滤掉已搜索的）
                pending_keywords = []
                ...
                if success:
                    ...
                    self.log_message("💡 脚本已生成完成，可以使用'▶️ 开始解析'按钮启动")
                    # 🔥 去掉弹窗：messagebox.showinfo("生成完成", result_msg)
                else:
                    self.log_message("❌ 脚本生成失败")
                    messagebox.showerror("错误", "脚本生成失败，请查看日志")

            except Exception as e:
                self.log_message(f"❌ 生成脚本异常: {e}")
    ...
    def start_parsing_scripts(self):
        ...
        def run_parsing():
            try:
                ...
                # 🔥 步骤1: 检查浏览器是否运行（通过API获取最新状态）
                if not self.api:
                    self.log_message("❌ API未连接")
                ...
                for i, script_info in enumerate(scripts_info):
                    try:
                        # 🔥 新的模块化结构：使用pdd_search_simple.py作为主启动脚本
                        ...
                        if main_script.exists():
                            ...
                            if os.name == 'nt':  # Windows系统
                                ...
                                startupinfo.wShowWindow = 0  # 🔥 修改：SW_HIDE改为正常显示，让控制台可见
                            ...
                            self.log_message(f"❌ 主启动脚本不存在: {main_script}")

                    except Exception as e:
                        self.log_message(f"❌ 启动脚本失败 {script_info.get('folder', 'unknown')}: {e}")
                ...
                if started_count > 0:
                    ...
                    self.log_message("❌ 没有成功启动任何脚本")

            except Exception as e:
                self.log_message(f"❌ 启动解析异常: {e}")
    ...
    def stop_parsing_scripts(self):
        ...
        try:
            ...
            if not script_processes:
                self.log_message("ℹ️ 没有找到运行中的采集脚本")
                # 🔥 修复：即使没有脚本进程，也要更新按钮状态
            ...
            if not safe_processes:
                self.log_message("ℹ️ 没有找到需要停止的脚本进程")
                self.root.after(0, self._on_stop_parsing_complete)
                return
            ...
            for proc in safe_processes:
                try:
                    ...
                    if not proc.is_running():
                        self.log_message(f"ℹ️ 进程 PID: {proc.pid} 已经停止")
                        continue
                    ...
                    if proc.is_running():
                        ...
                        self.log_message(f"ℹ️ 进程 PID: {proc.pid} 已自动停止")
                        stopped_count += 1
                    ...
                    try:
                        ...
                        if proc.is_running():
                            proc.kill()
                            self.log_message(f"🔥 强制终止脚本进程 PID: {proc.pid}")
                            stopped_count += 1
                        else:
                            self.log_message(f"ℹ️ 进程 PID: {proc.pid} 已停止")
                            stopped_count += 1
                    except Exception as kill_e:
                        self.log_message(f"❌ 强制杀死进程失败 PID {proc.pid}: {kill_e}")
                        # 🔥 最后尝试：使用taskkill强制终止（仅对脚本进程）
                        try:
                            import subprocess
                            ...
                            if result.returncode == 0:
                                ...
                                self.log_message(f"❌ taskkill终止失败 PID {proc.pid}: {result.stderr.decode()}")
                        except Exception as taskkill_e:
                            self.log_message(f"❌ taskkill执行失败 PID {proc.pid}: {taskkill_e}")
                except Exception as e:
            ...
            if stopped_count > 0:
                ...
                self.log_message("❌ 没有成功停止任何脚本")

        except ImportError:
            self.log_message("❌ 缺少psutil模块，无法停止脚本")
            messagebox.showerror("错误", "缺少psutil模块，无法停止脚本！\n请安装: pip install psutil")
        except Exception as e:
            self.log_message(f"❌ 停止脚本异常: {e}")
    ...
    def _unified_continue_system(self, target_browsers=None):
        ...
        try:
            ...
            for browser_id in target_browsers:
                try:
                    # 🔥 检查浏览器是否真的被暂停了
                    if self._is_browser_paused(browser_id):
                        # 确实被暂停了，删除标志文件并恢复进程
        ...
        try:
            if browser:
                ...
                if paused_count > 0:
                    ...
                    self.log_message("ℹ️ 没有需要暂停的进程")
                    
        except Exception as e:
            self.log_message(f"❌ 暂停程序失败: {e}")
    ...
    def _find_all_script_processes(self):
        ...
        try:
            ...
            self.log_message(f"❌ 查找所有脚本进程失败: {e}")
            return []
    ...
    def continue_program(self, browser=None):
        ...
        try:
            if browser:
                # 单浏览器继续
                browser_id = browser['id']
                self.log_message(f"▶️ 正在继续浏览器 {browser_id}...")
                ...
                if resumed_count > 0:
                    ...
                    self.log_message(f"ℹ️ 浏览器 {browser_id} 本来就在正常运行")
                else:
                    self.log_message(f"ℹ️ 浏览器 {browser_id} 没有需要恢复的进程")
            else:
                # 全局继续
                self.log_message("▶️ 正在继续所有采集脚本...")
                ...
                if resumed_count > 0:
                    ...
                    self.log_message(f"ℹ️ {skipped_count} 个浏览器本来就在正常运行")
                else:
                    self.log_message("ℹ️ 没有需要恢复的进程")

        except Exception as e:
            self.log_message(f"❌ 继续程序失败: {e}")

    def _resume_browser_processes(self, browser_id):
        """恢复浏览器被暂停的进程 - 单浏览器模式使用特定查找逻辑"""
        try:
            # 🔥 修复：单浏览器模式使用特定浏览器的进程查找逻辑
            script_processes = self._find_browser_script_processes(browser_id)
            ...
            if script_processes:
                ...
                if resumed_count > 0:
                    ...
                    self.log_message("ℹ️ 没有成功恢复任何进程")
            else:
                self.log_message("ℹ️ 未找到需要恢复的进程")
                
        except Exception as e:
            self.log_message(f"❌ 恢复浏览器进程失败: {e}")
    ...
    def check_simple_popup_alerts(self):
        """🔥 简化的弹窗检查（只检查popup_messages.json）"""
        try:
            import json
            from pathlib import Path
    ...
    def load_and_display_searched_keywords(self):
        ...
        try:
            ...
            if searched_keywords:
                ...
                print("ℹ️ 没有已搜索的关键词需要显示")

        except Exception as e:
            print(f"❌ 启动时加载已搜索关键词失败: {e}")
        ...
        try:
            ...
            def hide_browsers_thread():
                try:
                    ...
                    self.root.after(0, lambda: self.log_message(f"❌ 隐藏浏览器异常: {e}"))
        ...
        try:
            ...
            def show_browsers_thread():
                try:
                    ...
                    self.root.after(0, lambda: self.log_message(f"❌ 显示浏览器异常: {e}"))
        ...
        try:
            if results:
                ...
                if success_count > 0:
                    ...
                    self.log_message("❌ 所有浏览器显示失败")
            else:
                self.log_message("❌ 没有找到可显示的浏览器")
                
        except Exception as e:
            self.log_message(f"❌ 处理显示结果异常: {e}")
        ...
        try:
            if results:
                ...
                if success_count > 0:
                    ...
                    self.log_message("❌ 所有浏览器隐藏失败")
            else:
                self.log_message("❌ 没有找到可隐藏的浏览器")
                
        except Exception as e:
            self.log_message(f"❌ 处理隐藏结果异常: {e}")
    ...
    def open_details_folder(self):
        ...
        try:
            ...
            self.save_status_label.config(text=f"❌ 打开文件夹失败: {e}", foreground="red")
        ...
        try:
            if results:
                ...
                if success_count > 0:
                    ...
                    self.open_close_button.config(text="❌关闭所有", bg="#FF0000")
                else:
                    self.log_message("❌ 所有浏览器开启失败")
            else:
                self.log_message("❌ 没有找到可开启的浏览器")
                
        except Exception as e:
            self.log_message(f"❌ 处理开启结果异常: {e}")
        ...
        try:
            if results:
                ...
                if success_count > 0:
                    ...
                    self.log_message("❌ 所有浏览器关闭失败")
            else:
                self.log_message("❌ 没有找到可关闭的浏览器")
                
        except Exception as e:
            self.log_message(f"❌ 处理关闭结果异常: {e}")
    ...
    def _on_start_parsing_complete(self):
        ...
        try:
            ...
            self.log_message(f"❌ 更新开始解析按钮状态异常: {e}")
    ...
    def _on_stop_parsing_complete(self):
        ...
        try:
            ...
            self.start_stop_button.config(text="▶开始解析", bg="#2E8B57")
            self.log_message("✅ 解析已停止")
        except Exception as e:
            self.log_message(f"❌ 更新停止解析按钮状态异常: {e}")
    ...
    def _on_pause_program_complete(self):
        ...
        try:
            ...
            self.pause_continue_button.config(text="▶继续程序", bg="#2E8B57")
            self.log_message("✅ 程序已暂停")
        except Exception as e:
            self.log_message(f"❌ 更新暂停程序按钮状态异常: {e}")
    ...
    def _on_continue_program_complete(self):
        ...
        try:
            ...
            self.pause_continue_button.config(text="⏸暂停程序", bg="#FF0000")
            self.log_message("✅ 程序已继续")
        except Exception as e:
            self.log_message(f"❌ 更新继续程序按钮状态异常: {e}")
    ...
    def stop_program(self, browser):
        ...
        try:
            ...
            if script_processes:
                ...
                self.log_message("ℹ️ 未找到需要停止的脚本进程")
    ...
    def _force_kill_browser_process(self, browser_id):
        ...
        try:
            ...
            if killed_processes:
                ...
                self.log_message("ℹ️ 未找到需要终止的相关进程")
                
        except Exception as e:
            self.log_message(f"❌ 强制终止进程失败: {e}")
    ...
    def _update_browser_pause_status(self, browser_id, is_paused):
        ...
        try:
            ...
            self.log_message(f"❌ 更新浏览器暂停状态失败: {e}")
    ...
    def _update_browser_stop_status(self, browser_id, is_stopped):
        ...
        try:
            ...
            self.log_message(f"❌ 更新浏览器停止状态失败: {e}")
    ...
    def _create_pause_flag_file(self, browser_id):
        ...
        try:
            ...
            self.log_message(f"❌ 创建暂停标志文件失败: {e}")
    ...
    def _remove_pause_flag_file(self, browser_id):
        ...
        try:
            ...
            self.log_message(f"❌ 删除暂停标志文件失败: {e}")
